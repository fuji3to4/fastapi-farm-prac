...
async def login(request: Request, response: Response, user: UserBody,csrf_protect: CsrfProtect=Depends()):
    ...
    response.set_cookie(
        key="accsess_token",value=f"Bear {token}",httponly=True,samesite="none",secure=True
    )
    return {"message":"Successflly logged-in"}
...
def logout(request: Request, response: Response,csrf_protect: CsrfProtect=Depends()):
    ...
    csrf_protect.validate_csrf(csrf_token)
    response.set_cookie(
        key="accsess_token",value="",httponly=True,samesite="none",secure=True
    )
