...
async def update_tod(request: Request, response: Response, id:str, data:TodoBody, csrf_protect: CsrfProtect = Depends()):
    new_token = auth.verify_csrf_update_jwt(
        request,csrf_protect,request.headers
    )
    todo = jsonable_encoder(data)
    ...
    if res:
        return res
    raise HTTPException(
        status_code=404,detail=f"Update task failed for ID:{id}"
    )
...
async def delete_todo(request: Request, response: Response, id: str, csrf_protect: CsrfProtect = Depends()):
    ...
    raise HTTPException(
        status_code=404, detail=f"Delete task failed on ID:{id}"
    )
